...
import re,json
root=Path.cwd();changes=json.loads((root/'work/dialogue-dewford-20260910/changed-texts.json').read_text())
...
for row in changes:
 ...
 for line in re.split(r'\\[nlp]|\$',t):
  ...
  line=line.replace('{STR_VAR_1}', 'W'*28) # conservative two-word sample, not exhaustive Easy Chat proof
 ...
 rows.append(row)
(root/'work/dialogue-dewford-20260910/text-layout.json').write_text(json.dumps(rows,indent=2)+'\n')
