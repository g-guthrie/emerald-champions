...
paths=[p for p in (ROOT/'data/maps').glob('*/scripts.inc') if p.parent.name.startswith(('Dewford','GraniteCave'))]
rows=[]
for p in paths:
 s=p.read_text()
 for m in re.finditer(r'^(\w+):{1,2}\s*\n((?:\s*\.string[^\n]*\n)+)',s,re.M):
  ...
  rows.append(row)
(ROOT/'work/dialogue-dewford-20260910/dewford-text-layout.json').write_text(json.dumps(rows,indent=2,ensure_ascii=False)+'\n')
issues=[]
for r in rows:
 for l in r['lines']:
