...
paths=[ROOT/'data/maps'/name/'scripts.inc' for name in ['RusturfTunnel','Route116','Route104_MrBrineysHouse','DewfordTown_Gym']]
rows=[]
for p in paths:
 s=p.read_text()
 for m in re.finditer(r'^(\w+):{1,2}\s*\n((?:\s*\.string[^\n]*\n)+)',s,re.M):
  ...
  rows.append(row)
(ROOT/'work/dialogue-dewford-20260910/handoff-text-layout.json').write_text(json.dumps(rows,indent=2,ensure_ascii=False)+'\n')
issues=[]
for r in rows:
 for l in r['lines']:
