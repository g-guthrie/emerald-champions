...
static const u8 *GetLegendaryGuideStatus(enum LegendarySignId id)
{
    ...
    if (!MeetsSignProgression(id))
        return COMPOUND_STRING(" (Not yet)");
    if (!MeetsSignDiscovery(id))
        return COMPOUND_STRING(" (Needs partner)");
    ...
    case LEGENDARY_SOURCE_RARE_WILD:
        return COMPOUND_STRING(" (Available)");
    default:
...
// Normal encounters give each eligible local species its own 1% outcome.
// Sweet Scent has one 25% legendary roll, then chooses eligible species evenly.
enum Species ChooseRareWildLegendarySpecies(enum WildPokemonArea area, bool32 sweetScent)
{
    ...
    enum Species eligible[LEGENDARY_SIGN_COUNT];
    u32 count = 0;
    u32 roll;

    if (map == MAP_ROUTE125 || map == MAP_ROUTE126 || map == MAP_ROUTE127)
        habitat = WILD_AREA_WATER;
    // Seabed encounters use the underwater map's twelve-slot land table.
    if (area != habitat)
        return SPECIES_NONE;
    for (enum LegendarySignId id = 0; id < LEGENDARY_SIGN_COUNT; id++)
    {
        const struct LegendarySignDefinition *sign = &gLegendarySignDefinitions[id];
        if (sign->source == LEGENDARY_SOURCE_RARE_WILD && sign->mapId == map
         && CanAcquireLegendarySignSpecies(sign->species))
            eligible[count++] = sign->species;
    }
    if (count == 0)
        return SPECIES_NONE;
    if (sweetScent)
    {
        if (RandomUniform(RNG_NONE, 0, 3) != 0)
            return SPECIES_NONE;
        return eligible[RandomUniform(RNG_NONE, 0, count - 1)];
    }
    roll = RandomUniform(RNG_NONE, 0, 99);
    return roll < count ? eligible[roll] : SPECIES_NONE;
}

// Existing area-information signs can show up-to-date local discovery leads.
// Their status comes from the same requirements that select wild encounters.
void BuildLocalLegendaryResearchMenu(void)
{
    u16 map = ((u8)gSaveBlock1Ptr->location.mapGroup << 8) | (u8)gSaveBlock1Ptr->location.mapNum;
    gSpecialVar_Result = 0;
    for (enum LegendarySignId id = 0; id < LEGENDARY_SIGN_COUNT; id++)
...
static void BufferNativeLegendaryLead(u32 index)
{
    ...
    if (species == SPECIES_GROUDON || species == SPECIES_KYOGRE)
        StringAppend(gStringVar4, COMPOUND_STRING("\pAfter resolving the weather crisis,\nask the Weather Institute about\lunusual weather. Follow its report."));
        ...
        StringAppend(gStringVar4, COMPOUND_STRING("\pWith six badges, ask the sailor at\nLilycove Harbor about his sightings.\pOne roams Hoenn; the other waits on\nSouthern Island. He gives the passes."));
...
void ResearchSelectedLegendarySign(void)
{
    ...
    if (!MeetsSignProgression(id))
    {
        if (GetBadgeCountForLegendarySigns() < sign->minimumBadges)
        {
            ...
            u8 buffer[80];
            StringExpandPlaceholders(buffer, COMPOUND_STRING("\pAvailable after earning\n{STR_VAR_1} Gym Badges."));
        ...
        else if (sign->requiredFlag == FLAG_HIDE_WEATHER_INSTITUTE_2F_AQUA_GRUNT_M && !FlagGet(sign->requiredFlag))
    ...
    case LEGENDARY_SOURCE_RARE_WILD:
        ...
        StringAppend(gStringVar4, COMPOUND_STRING("\pSWEET SCENT gives a 25% chance of\na legendary encounter here.\pAvailable legends share that chance.\nIt cannot bypass their requirements."));
