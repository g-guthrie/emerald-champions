...
bool32 TryNormalizeEmeraldChampionsBellyDrumHpParity(struct Pokemon *mon)
{
    ...
    for (u32 i = 0; i <= 2 * EC_BELLY_DRUM_HP_EV_TOLERANCE / 4; i++)
    {
        s32 offset = 4 * ((i + 1) / 2);
...
static bool32 DoesMonMatchPreset(struct Pokemon *mon, const struct EmeraldChampionsBattleSet *preset)
{
    ...
    for (u32 stat = 0; stat < NUM_STATS; stat++)
    {
        ...
        if (stat == 0 && diff >= -EC_BELLY_DRUM_HP_EV_TOLERANCE
         && diff <= EC_BELLY_DRUM_HP_EV_TOLERANCE && DoesPresetWantEvenHp(preset))
            continue;
        if (diff != 0)
            return FALSE;
    }
