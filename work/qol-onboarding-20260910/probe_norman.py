...
out=Path('work/qol-onboarding-20260910');sy=symbols(Path('pokeemerald-qol-onboarding-headless.elf'),Path.cwd(),first=True);(out/'symbols.json').write_text(json.dumps(sy))
...
for param in [16]:
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-qol-onboarding-headless.gba','--frames',str(18000 if param==16 else 4800),'--screenshot',str(out/f'scene-{param}.png'),'--state-out',str(out/f'scene-{param}.state')]
