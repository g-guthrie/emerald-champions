...
for param,name,kind in [(44,'FLAG_RUSTURF_TUNNEL_OPENED',1),(45,'FLAG_RUSTURF_TUNNEL_OPENED',1)]:
 ...
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','9000','--screenshot',str(out/f'reunion-{param}-end.png')]
 ...
 cmd+=['--key','300:32:'+{44:'RIGHT',45:'LEFT'}[param],'--key','400:2:A']
 
 for frame in [750,1600,2400,4000,8000]:cmd+=['--screenshot-at',f'{frame}:{out}/reunion-{param}-{frame}.png']
 for frame in range(600,8500,100):cmd+=['--key',f'{frame}:2:'+('A' if frame < 4500 else 'B')]
 ...
 result=subprocess.run(cmd,capture_output=True,text=True);(out/f'reunion-{param}.log').write_text(result.stdout+result.stderr);print(param,name,result.returncode,result.stdout,flush=True)
