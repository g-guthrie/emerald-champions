...
program=bytes([0x39,0,15,255])+struct.pack('<HH',8,18)+bytes([0x27,0x6b,2]);addr=sy['gStringVar4']+800;ctx=sy['sGlobalScriptContext']
c=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--state-in',str(out/'regi-a-base.mgba'),'--frames','6500','--state-out',str(out/'pacifidlog-bridge-crossed.mgba'),'--screenshot',str(out/'pacifidlog-bridge-end.png')]
...
keys=[(700+150*i,16,'DOWN') for i in range(9)]+[(2200+150*i,16,'RIGHT') for i in range(9)]+[(3800+150*i,16,'UP') for i in range(6)]
for f,d,k in keys:c+=['--key',f'{f}:{d}:{k}']
for f in [650,705,715,735,1800,2050,2210,2250,3650,4700]:c+=['--screenshot-at',f'{f}:{out}/pacifidlog-bridge-{f}.png']
