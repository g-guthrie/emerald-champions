...
for param,name,kind in [(72,'FLAG_BADGE04_GET',1)]:
 ...
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','16000','--screenshot',str(out/f'lavaridge-buried-{param}-end.png')]
 ...
 for frame in [750,2000,4000,6000,8000]:cmd+=['--screenshot-at',f'{frame}:{out}/lavaridge-buried-{param}-{frame}.png']
 ...
 result=subprocess.run(cmd,capture_output=True,text=True);(out/f'lavaridge-buried-{param}.log').write_text(result.stdout+result.stderr);print(param,name,result.returncode,result.stdout,flush=True)
