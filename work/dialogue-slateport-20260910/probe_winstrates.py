...
for param,name,kind in [(48,'FLAG_HIDE_ROUTE_111_VICKY_WINSTRATE',1)]:
 ...
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','40000','--screenshot',str(out/f'winstrates-{param}-end.png')]
 ...
 cmd+=['--key','300:12:UP']
 if param!=31:cmd+=['--key','400:2:A']
 for frame in [750,2000,4000,6000,8000]:cmd+=['--screenshot-at',f'{frame}:{out}/winstrates-{param}-{frame}.png']
 for frame in range(1000,39000,100):cmd+=['--key',f'{frame}:2:'+'A']
 for n in ['gEcHeadlessCampaignQueryValue','gEcHeadlessCampaignControlsLocked','gEcHeadlessCampaignScriptEnabled','gEcHeadlessCampaignMapId','gEcHeadlessCampaignBattleSerial','gEcHeadlessCampaignLastResolution']:cmd+=['--read',f'4:{sy[n]}']
 result=subprocess.run(cmd,capture_output=True,text=True);(out/f'winstrates-{param}.log').write_text(result.stdout+result.stderr);print(param,name,result.returncode,result.stdout,flush=True)
