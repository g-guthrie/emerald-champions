...
paths=sorted((ROOT/'data/maps').glob('SlateportCity*/scripts.inc'))
rows=[]
for p in paths:
 s=p.read_text()
 for m in re.finditer(r'^(\w+):{1,2}\s*\n((?:\s*\.string[^\n]*\n)+)',s,re.M):
  ...
  rows.append(row)
(ROOT/'work/dialogue-slateport-20260910/text-layout.json').write_text(json.dumps(rows,indent=2,ensure_ascii=False)+'\n')
issues=[]
for r in rows:
 for l in r['lines']:
