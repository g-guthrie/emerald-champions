...
for param,name,kind in [(69,'FLAG_BADGE04_GET',1),(70,'FLAG_BADGE04_GET',1)]:
 ...
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','16000','--screenshot',str(out/f'lavaridge-gym-{param}-end.png')]
 ...
 for frame in [750,2000,4000,6000,8000]:cmd+=['--screenshot-at',f'{frame}:{out}/lavaridge-gym-{param}-{frame}.png']
 ...
 result=subprocess.run(cmd,capture_output=True,text=True);(out/f'lavaridge-gym-{param}.log').write_text(result.stdout+result.stderr);print(param,name,result.returncode,result.stdout,flush=True)
