...
paths=[ROOT/'data/text/trainers.inc']
rows=[]
for p in paths:
 s=p.read_text()
 for m in re.finditer(r'^(\w+):{1,2}\s*\n((?:\s*\.string[^\n]*\n)+)',s,re.M):
  if not m[1].startswith('Route117_Text_'):continue
  ...
  rows.append(row)
(ROOT/'work/dialogue-slateport-20260910/route117-trainer-text-layout.json').write_text(json.dumps(rows,indent=2,ensure_ascii=False)+'\n')
issues=[]
for r in rows:
 for l in r['lines']:
