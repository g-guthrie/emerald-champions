...
for puzzle in [1,2]:
 ...
 blocked.discard((2,2));blocked.discard((2,1));walk_to({(2,1)});frame+=600
 d=json.loads(Path('data/maps/Route110_TrickHouseCorridor/map.json').read_text());l=next(l for l in layouts if l['id']==d['layout']);w,h=l['width'],l['height'];raw=Path(l['blockdata_filepath']).read_bytes();grid=list(struct.unpack('<'+'H'*(len(raw)//2),raw));blocked={(13,3),(14,3)};trees=set();pos=(13,3)
 shots+=['--screenshot-at',f'{frame-100}:{out}/trick-exit-{puzzle}-corridor.png'];walk_to({(4,23)});key('DOWN');frame+=600
 shots+=['--screenshot-at',f'{frame-100}:{out}/trick-exit-{puzzle}-entrance.png']
 # Read the actual reward quantity after the corridor return.
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames',str(frame+500),'--screenshot',str(out/f'trick-exit-{puzzle}-end.png')]
 for f,n,v in [(59,'gEcHeadlessFixtureParam',45+puzzle),(60,'gEcHeadlessFixtureScenario',71),(70,'gEcHeadlessCampaignQueryKind',4),(70,'gEcHeadlessCampaignQueryId',862 if puzzle==1 else 11)]:cmd+=['--write',f'{f}:4:{sy[n]}:{v}']
 cmd+=commands+shots
 ...
 (out/f'trick-exit-{puzzle}-itinerary.json').write_text(json.dumps({'actions':actions,'frames':frame+500,'cmd':cmd},indent=2)+'\n')
