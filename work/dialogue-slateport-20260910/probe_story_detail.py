...
for param,name,kind in [(32,'FLAG_DEFEATED_WALLY_MAUVILLE',1)]:
 ...
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','18000','--screenshot',str(out/f'mauville-{param}-end.png')]
 ...
 for frame in [1100,1300,1500,1700,1900]:cmd+=['--screenshot-at',f'{frame}:{out}/mauville-{param}-{frame}.png']
 for frame in range(1000,17000,100):cmd+=['--key',f'{frame}:2:'+('A' if frame < 5000 else 'B')]
 for n in ['sStorytellerPtr','gEcHeadlessCampaignQueryValue','gEcHeadlessCampaignControlsLocked','gEcHeadlessCampaignScriptEnabled','gEcHeadlessCampaignMapId']:cmd+=['--read',f'4:{sy[n]}']
 result=subprocess.run(cmd,capture_output=True,text=True);(out/f'mauville-{param}.log').write_text(result.stdout+result.stderr);print(param,name,result.returncode,result.stdout,flush=True)
