...
for param,name,kind in [(p,'VAR_LAVARIDGE_TOWN_STATE',2) for p in (map(int,sys.argv[1:]) if len(sys.argv)>1 else range(64,69))]:
 ...
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','9000','--screenshot',str(out/f'lavaridge-rival-{param}-end.png')]
 ...
 for frame in [750,2000,4000,6000,8000]:cmd+=['--screenshot-at',f'{frame}:{out}/lavaridge-rival-{param}-{frame}.png']
 for frame in range(1000,8500,100):cmd+=['--key',f'{frame}:2:'+'B']
 ...
 result=subprocess.run(cmd,capture_output=True,text=True);(out/f'lavaridge-rival-{param}.log').write_text(result.stdout+result.stderr);print(param,name,result.returncode,result.stdout,result.stderr,flush=True)
