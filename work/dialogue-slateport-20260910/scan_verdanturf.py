...
paths=sorted((ROOT/'data/maps').glob('Verdanturf*/scripts.inc')) + sorted((ROOT/'data/maps').glob('Route117*/scripts.inc'))
rows=[]
for p in paths:
 s=p.read_text()
 for m in re.finditer(r'^(\w+):{1,2}\s*\n((?:\s*\.string[^\n]*\n)+)',s,re.M):
  ...
  for t in re.split(r'\\[nlp]|\$',text):
   ...
   def expand(k):
    ...
    if name.startswith('STR_VAR'):
     if 'FriendshipRatersHouse_Text_' in m[1]: return 'CHANSEY' if name=='STR_VAR_2' else 'IGGLYBUFF'
  ...
  rows.append(row)
(ROOT/'work/dialogue-slateport-20260910/verdanturf-text-layout.json').write_text(json.dumps(rows,indent=2,ensure_ascii=False)+'\n')
issues=[]
for r in rows:
 for l in r['lines']:
