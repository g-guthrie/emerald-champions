...
paths=[ROOT/'data/scripts/day_care.inc']
rows=[]
for p in paths:
 s=p.read_text()
 for m in re.finditer(r'^(\w+):{1,2}\s*\n((?:\s*\.string[^\n]*\n)+)',s,re.M):
  ...
  for t in re.split(r'\\[nlp]|\$',text):
   ...
   def expand(k):
    ...
    if name.startswith('STR_VAR'):
     if name=='STR_VAR_1': return 'W'*12
     if name=='STR_VAR_3': return 'W'*12
     if 'FriendlyWithOther' in m[1]: return 'W'*7
     return '99999'
  ...
  rows.append(row)
(ROOT/'work/dialogue-slateport-20260910/daycare-text-layout.json').write_text(json.dumps(rows,indent=2,ensure_ascii=False)+'\n')
issues=[]
for r in rows:
 for l in r['lines']:
