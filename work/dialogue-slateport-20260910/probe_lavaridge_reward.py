...
for param,name,kind in [(69,'FLAG_BADGE04_GET',1)]:
 ...
 cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','16000','--screenshot',str(out/f'lavaridge-reward-{param}-end.png')]
 ...
 cmd+=['--key','300:2:'+('UP' if param==69 else 'DOWN')]
 if param>=62:cmd+=['--key','400:2:A']
 for frame in [750,2000,4000,6000,8000]:cmd+=['--screenshot-at',f'{frame}:{out}/lavaridge-reward-{param}-{frame}.png']
 for frame in range(1000,15500,100):cmd+=['--key',f'{frame}:2:'+'B']
 cmd+=['--write',f'15550:4:{sy["gEcHeadlessCampaignQueryKind"]}:4','--write',f'15550:4:{sy["gEcHeadlessCampaignQueryId"]}:323']
 ...
 result=subprocess.run(cmd,capture_output=True,text=True);(out/f'lavaridge-reward-{param}.log').write_text(result.stdout+result.stderr);print(param,name,result.returncode,result.stdout,flush=True)
