...
def run(case):
 ...
 c=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--state-in',str(out/'victory-audit-base.mgba'),'--frames','6500','--screenshot',str(out/(tag+'-end.png'))]
 ...
 for f,d,k in [(700,2,'UP'),(900,2,'A')]+[(f,2,'B') for f in range(1700,6100,650)]:c+=['--key',f'{f}:{d}:{k}']
...
with concurrent.futures.ThreadPoolExecutor(max_workers=2) as ex:list(ex.map(run,[(False,False),(False,True),(True,False),(True,True)]))
