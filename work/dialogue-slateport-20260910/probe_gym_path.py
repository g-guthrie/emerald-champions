...
start=(4,19,0,0);todo=deque([start]);prev={start:None};directions=[('UP',0,-1),('RIGHT',1,0),('DOWN',0,1),('LEFT',-1,0)];end=None
while todo:
 p=todo.popleft();x,y,state,last=p
 if (x,y)==(5,3):end=p;break
...
(out/'gym-planned-route.json').write_text(json.dumps(route,indent=2)+'\n')
sy=symbols(Path('pokeemerald-mauville-audit-headless.elf'),Path.cwd(),first=True)
cmd=['build/headless/emerald_champions_mgba_runner','--rom','pokeemerald-mauville-audit-headless.gba','--frames','5000','--screenshot',str(out/'gym-path-end.png')]
...
for i,(key,pos) in enumerate(route):
 ...
 if i==0 or (pos[0],pos[1]) in switches or i==len(route)-1:cmd+=['--screenshot-at',f'{f+65}:{out}/gym-path-step-{i}.png']
