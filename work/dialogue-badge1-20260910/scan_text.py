...
paths=[p for p in (ROOT/'data/maps').glob('*/scripts.inc') if p.parent.name.startswith(('Petalburg','Rustboro','Route104')) and 'Prototype' not in p.parent.name]+[ROOT/'data/maps/Route102/scripts.inc',ROOT/'data/maps/Route116/scripts.inc',ROOT/'data/maps/Route116_TunnelersRestHouse/scripts.inc',ROOT/'data/maps/RusturfTunnel/scripts.inc',ROOT/'data/text/berries.inc',ROOT/'data/text/trainers.inc']
rows=[]
for p in paths:
 s=p.read_text()
 for m in re.finditer(r'^(\w+):{1,2}\s*\n((?:\s*\.string[^\n]*\n)+)',s,re.M):
  ...
  rows.append(row)
(ROOT/'work/dialogue-badge1-20260910/text-inventory.json').write_text(json.dumps(rows,indent=2,ensure_ascii=False)+'\n')
issues=[]
for r in rows:
 for l in r['lines']:
