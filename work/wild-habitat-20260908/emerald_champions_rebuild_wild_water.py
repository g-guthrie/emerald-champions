#!/usr/bin/env python3
"""Materialize existing water/fishing tables from wild_route_sheet.json.

Species selection uses west, inland, east, deep and ice regional pools plus
authored anchors and overrides. Actual probability weights belong to the
fields in wild_encounters.json; the old 1% hunt descriptions are obsolete.
The current ordinary minimum is 5%, except Feebas. Surf uses 35/25/18/12/10;
the three rods use 60/40, 45/30/25 and 30/25/20/15/10.

Table levels describe authoring tiers, not physical access. Runtime clamps
levels to the current badge cap but does not demote an evolved species.
This script therefore cannot establish when a form first becomes usable.

The historical land placement, pinned-slot, evolution-floor and deduplication
passes below also run in main(). They currently leave the authored land data
unchanged, but are not a design brief for future redistribution. Review the
whole diff when regenerating; the build itself consumes wild_encounters.json.
"""
