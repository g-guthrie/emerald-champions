...
TEST("EC trainer levels: wide offsets survive creation, Mega stats and both opponent owners")
{
    ...
    for (u32 i = 0; i < ARRAY_COUNT(owners); i++)
    {
        ...
        EXPECT_EQ(GetMonData(&party[0], MON_DATA_LEVEL), 150);
        EXPECT_EQ(GetMonData(&party[1], MON_DATA_LEVEL), 250);
        EXPECT_EQ(GetMonData(&party[2], MON_DATA_LEVEL), 80);
    ...
    EXPECT_EQ(GetCampaignTrainerLevel(50), 152);
    SetCurrentDifficultyLevel(DIFFICULTY_EASY);
    EXPECT_EQ(GetCampaignTrainerLevel(50), 148);
