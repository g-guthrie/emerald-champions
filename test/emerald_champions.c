        ...
        for (enum Species species = SPECIES_BULBASAUR; species < NUM_SPECIES; species++)
        {
            ...
            PARAMETRIZE_LABEL("format=%d species=%d", format, species)
            {
                ...
                for (u32 raw = 0; raw <= ranges[species].count; raw++)
                {
                    ...
                    // Belly Drum's berry normalizer adjusts only HP EVs within
                    // a bounded neighborhood; it does not redistribute EVs.
                    // Every other stat must still match the authored spread.
                    for (u32 stat = 0; stat < NUM_STATS; stat++)
                    {
                        u32 points = GetMonData(&mon, EC_EV_DATA(stat));
                        total += points;
                        if (stat != 0 || points == preset->evs[0])
                        ...
                        else
                        {
                            ...
                            EXPECT_GE((s32)points, (s32)preset->evs[0] - 24);
                            EXPECT_LE(points, preset->evs[0] + 24);
...
TEST("Emerald Champions Belly Drum berry sets preserve Sitrus points and Gluttony parity at every level cap")
{
    ...
    for (u32 s = 0; s < ARRAY_COUNT(species); s++)
    {
        for (u32 c = 0; c < ARRAY_COUNT(sEmeraldChampionsLevelCaps); c++)
        {
            ...
            {
                for (u32 stat = 0; stat < NUM_STATS; stat++)
                    EXPECT_EQ(GetMonData(mon, EC_EV_DATA(stat)), preset->evs[stat]);
            }
            else
            {
