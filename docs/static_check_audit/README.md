...
September10 Jacob follow-up: retain one native regression for repeated Leech
Seed into an already-seeded Follow Me recipient. Frozen native parties use
unforced AI choices; assertions require a successful initial Seed and a useful
next attack. Reverting only the new redirection-risk penalty fails with Seed
PP8 instead of9 (`work/jacob-seed-regression-counterfactual.log`); fixed test
passes. Restored correction and scratch-free shared suite pass55/55 in
`work/ai-shared-through-jacob.log`. No trainer-specific permanent scenario
corpus or broad redirection-correctness claim is added.

September10 Benjamin follow-up: one retained native regression protects the
demonstrated Tailwind rewards for unusable Electric charge and fainted Speed
targets. The frozen old-party board passes with the correction; reverting
only the two relevant scoring changes spends Tailwind PP and fails the
assertion in `work/benjamin-wind-regression-counterfactual.log`. With the
correction restored and encounter scratch excluded,54/54 shared groups pass
in `work/ai-shared-through-benjamin.log`. Useful Wind Power charge/attack
execution is separately observed in temporary authoring scenarios. No static
gate, new test file or permanent per-trainer corpus was added.

September10 Jaclyn follow-up: one retained native regression protects the
demonstrated Calm Mind immediate-special-defense omission in the pair evaluator.
The frozen native-stat board uses unforced moves; removing only Calm Mind's
support from the shared defensive helper changes its choice to Air Slash and
fails the required setup observation. After restoring the correction and
excluding encounter scratch, all53 retained shared groups pass in
`work/ai-shared-through-jaclyn.log`. This adds no static gate, broad strategy
quota or permanent per-encounter corpus. The historical host counts stay below.

September10 Alyssa follow-up: one necessary native Tailwind regression was
added to the existing `test/battle/ai/emerald_champions_plans.c` owner. It
reproduces a real faster-than-everyone board wasting Tailwind; removing only
the shared no-payoff penalty makes its PP assertion fail (14 rather than15).
The repaired check and all52 retained shared groups pass in
`work/ai-shared-through-alyssa.log`. Encounter scratch scenarios are excluded.
This is distinct failure protection, not a new static gate or per-team corpus;
it does not change the historical host-audit counts below.
