...
## Systems consolidation — September 16, 2026

Eight read-only Opus audits (work/systems-audit/01..08, merged design in 00-DESIGN.md) followed by seven Opus implementation worktrees merged into main, with every map, warp, layout and object change done by hand and verified natively. Decisions: Mauville starters cost 10,000 money; Shoal Cave's tide is turned on request by Shoalmaster Fen (no clock); Match Call is removed.

Landed: former HM moves can be forgotten; Summary shows the tutor-set Nature and cycles IVs/EVs; Pay Day pays only where prize money does and Mom's Amulet Coin is gone; TMs and HMs deleted as a system (113 items, the pocket, the machine API, the relearner categories) and the Bag is seven pockets with "Held Items"; Match Call, its Rustboro tutorial, the PokéNav Condition tab, contests, Pokéblocks, blenders, the Safari Pokéblock gate, the Center second floors (escalator tiles replaced, sixteen warps removed) and Mystery Gift removed; the Game Corner is the Mauville Starter Archive (no coins, no gambling); Trainer Hill sealed with no prizes; the six retired Frontier facilities are Circuit desks and their doors are closed; one free Bicycle; Day Care free; Name Rater, Move Deleter house and Fallarbor EV programme redirect to the Center; lottery once per Circuit run with cosmetic prizes; Shoal Cave tide-on-request with Glalitite, a charted guaranteed ice-room legendary, decorations and no Big Pearl; berry gifts mint pouch credit and Scott's berries are gone; soot vestiges fixed; dead Mart stock removed; treasure sells for 0 and all 53 treasure pickups (12 balls, 41 hidden) hold Balls or evolution tools; prize multipliers 2/4/10/16/20 with docs/economy-ledger.md (lifetime ≈118,000 vs a 46,000 catalogue); Options with Text speed and a difficulty explanation; Trainer Card back records this game (Megas witnessed bitfield in SaveBlock1 filler); the nurse's first speech trimmed and the Ball top-up stated; Circuit and Tent levels now carry the same +2 difficulty compensation as the campaign (Hard plays at 100 again). Save layout changed (bag pockets, item ids); no earned run was active.

Cleanup wave (same day, five Opus worktrees, merged by hand): contests, Pokéblock feeding and condition screens, the Berry Blender, the six retired Frontier facilities with their 32 interior maps, the Apprentice and Reporter, Trainer Hill and its seven maps, the e-Reader, Match Call, Mystery Gift, and the seventeen Center second floors (League 2F included, its escalator tiled over) are deleted from the tree, about 120,000 lines. Deliberately kept: slots and roulette (owner decision: they play for money behind the Coin Case), the Battle Tower chain the Circuit uses, src/pokeblock.c and SaveBlock1.pokeblocks (the Safari battle menu's throw runs through the battle engine), the Frontier Brain data and facility index constants (positional save-block indices), link/union-room/cable-club infrastructure (FRLG maps bind it), and the Palace nature-move code in the engine. ITEM_MACH_BIKE stays defined with no producer.
