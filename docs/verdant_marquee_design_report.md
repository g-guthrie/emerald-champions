...
- Status: `authored-draft` / critic `pending` / source `unimplemented` / runtime `unplayed`
- Format: `double`
- Strict cap: 80
...
| 3 | `SPECIES_YVELTAL` | +1 | `ITEM_SITRUS_BERRY` | `ABILITY_DARK_AURA` | `MOVE_TAILWIND`, `MOVE_OBLIVION_WING`, `MOVE_SNARL`, `MOVE_PROTECT` | First tempo reserve; Dark Aura, Tailwind, Snarl, and Oblivion Wing establish the middle game without becoming the climax. |
| 4 | `SPECIES_ABSOL` | +1 | `ITEM_ABSOLITE` | `ABILITY_SUPER_LUCK` | `MOVE_KNOCK_OFF`, `MOVE_PLAY_ROUGH`, `MOVE_WILL_O_WISP`, `MOVE_PROTECT` | Only Mega and proactive disruption phase; removes items, burns physical attackers, reflects status, and attacks without requiring screens or a Protect prediction. |
| 5 | `SPECIES_DARKRAI` | +1 | `ITEM_CHOICE_SPECS` | `ABILITY_BAD_DREAMS` | `MOVE_DARK_PULSE`, `MOVE_SLUDGE_BOMB`, `MOVE_ICE_BEAM`, `MOVE_THUNDERBOLT` | Committal special coverage bridge, not a setup endgame; its Choice lock is visible counterplay and the selector uses it only into a visibly favorable board. |
| 6 | `SPECIES_KINGAMBIT` | +2 | `ITEM_BLACK_GLASSES` | `ABILITY_SUPREME_OVERLORD` | `MOVE_KOWTOW_CLEAVE`, `MOVE_SWORDS_DANCE`, `MOVE_SUCKER_PUNCH`, `MOVE_PROTECT` | Sole final setup climax; Supreme Overlord converts the visible allied faint count into a slow, answerable last stand. |
...
- Secondary mode: Mega Absol proactively removes items, burns physical attackers, and reflects status on ordinary boards; Choice Specs Darkrai is a locked special coverage bridge, while Kingambit alone owns the setup climax.
- Difficulty rationale: The player cap remains 80 while Sidney now uses offsets 0, 0, +1, +1, +1, and +2. Difficulty must come from the opening target dilemma, interactable Tailwind, proactive Mega Absol disruption, an exploitable Choice Specs lock, and the visible Supreme Overlord climax rather than a +4 Mega or three unrelated setup sweepers. A blind loss is expected, but every escalation teaches a reusable adjustment.
- First-loss lesson: Do not treat Protect as universal against the opener. Reveal and exploit Urshifu and Darkrai locks, contest Tailwind, pressure Mega Absol on its ordinary disruption turns, and preserve Fighting, Ground, Haze, or phazing for the visibly empowered final Kingambit.
- Intentional weakness: Sidney has no redirection or screens. Urshifu and Darkrai expose Choice locks, Tailwind is interactable, Mega Absol is frail and has only one sustain move, and Kingambit remains 4x weak to Fighting and must spend a visible turn to Swords Dance.

### AI and evidence

- State machine: [{'state': 'steal-position', 'condition': 'Opening Incineroar plus Urshifu board', 'behavior': 'Use Fake Out, Knock Off, Wicked Blow, Iron Head, U-turn, Flare Blitz, or Parting Shot from current damage, typing, and revealed actions; no fixed combination.'}, {'state': 'aura-tempo', 'condition': 'A reserve is required and Tailwind is absent with healthy Yveltal', 'behavior': 'Select Yveltal, establish Tailwind when it survives, then use Dark Aura, Snarl, and Oblivion Wing to shape the next visible reserve decision.'}, {'state': 'mega-disruption', 'condition': 'Tailwind has been established or Yveltal has left, Absol is healthy, and at least one visible foe does not resist both Dark and Fairy', 'behavior': 'Select Absol, Mega Evolve automatically, and proactively choose Knock Off, Will-O-Wisp, Play Rough, or Protect from ordinary board value.'}, {'state': 'specs-bridge', 'condition': 'Visible active typing or public species defenses make special coverage stronger than Mega Absol dual STAB', 'behavior': 'Select Darkrai, commit to the best legal Choice Specs attack, reveal the lock, and switch only from visible state if the lock becomes dead.'}, {'state': 'overlord-climax', 'condition': 'At least three Sidney allies have visibly fainted, or Kingambit is the only healthy legal reserve', 'behavior': 'Select Kingambit, use the actual Supreme Overlord multiplier, set Swords Dance only on a defensible turn, and finish through Kowtow Cleave or Sucker Punch.'}]
...
- Required source work: Preserve the Incineroar plus Urshifu opening and score Fake Out, Wicked Blow, U-turn, Parting Shot, and direct damage from the actual board without a forced target.; Implement the visible-state healthy-reserve selector exactly as recorded in ordering; raw array order is never sufficient.; Prefer Yveltal only while Tailwind is absent and it can survive the visible board; do not waste Tailwind while active.; Mega Absol must use Knock Off against a removable held item, Will-O-Wisp against a visible physical threat that can be burned, Play Rough for damage, and Protect only for an actual defensive turn.; Choice Specs Darkrai must select from four direct attacks, expose its lock, and smart-switch only from current type effectiveness, current HP, known field state, and revealed information.; Include Supreme Overlord current faint multiplier in damage estimates and withhold Kingambit until the visible faint-count climax unless it is the only legal reserve.; Evaluate Kingambit Swords Dance only when it survives the current visible board; it is the only setup scorer in the roster.; Continue using automatic opponent Mega Evolution; Absol is the only Mega candidate.

### Fresh-context verdict

- Final fresh no-history review is pending.
...
| 2 | `SPECIES_FLUTTER_MANE` | +1 | `ITEM_FOCUS_SASH` | `ABILITY_PROTOSYNTHESIS` | `MOVE_PERISH_SONG`, `MOVE_MOONBLAST`, `MOVE_SHADOW_BALL`, `MOVE_PROTECT` | Second singer and immediate special pressure; imported nearly exactly from Wolfe Glick's Orlando-winning sheet. |
| 3 | `SPECIES_AMOONGUSS` | +2 | `ITEM_MENTAL_HERB` | `ABILITY_REGENERATOR` | `MOVE_RAGE_POWDER`, `MOVE_SPORE`, `MOVE_SYNTHESIS`, `MOVE_PROTECT` | The one off-type ritual guardian; buys finite turns, heals itself, and can cycle through Regenerator. |
...
- State machine: [{'state': 'secure-or-break-escape', 'condition': 'Mega Gengar is not yet active, a foe is visibly Ghost-type, or an escape ability, item, or pivot move has already been revealed', 'behavior': 'Mega Evolve automatically; use Shadow Ball, Disable, Spore, Will-O-Wisp, Taunt, or direct focus fire until the clock can teach a fair actionable lesson. Never inspect an unrevealed player move or item for this transition.'}, {'state': 'start-clock', 'condition': "At least one living foe is unperished and Mega Gengar's Shadow Tag affects the relevant slots", 'behavior': 'Give exactly one singer a strong Perish Song preference while the partner attacks, Protects, or disrupts according to the board.'}, {'state': 'survive-three-two', 'condition': 'A useful own-side Perish count is two or three', 'behavior': "Value Protect, Disable, Rage Powder, Spore, Synthesis, Substitute, King's Shield, and safe switching without forcing a deterministic sequence."}, {'state': 'exit-at-one', 'condition': "An AI Pokemon's count is one and a legal reserve exists", 'behavior': 'Switch it. If trapped or no reserve exists, choose the highest-value attack or protection rather than pretending survival is possible.'}, {'state': 'second-circle', 'condition': 'The first opposing pair has fainted and Phoebe retains a singer plus Mega Gengar', 'behavior': 'Re-establish Shadow Tag and a new Perish count while preserving enough reserves to exit once more.'}, {'state': 'direct-cleanup', 'condition': 'The trapper has fainted, revealed escape counterplay remains, or only damaged foes survive', 'behavior': 'Use the trainer-aware reserve selector to transition to non-Protect Mimikyu and four-attack Marshadow, with Flutter Mane or Aegislash attacking as appropriate, instead of wasting turns on an unenforceable clock.'}]
...
- Required source work: Recognize Mega Gengar's post-Mega Shadow Tag when deciding whether either singer should start Perish Song.; Reject duplicate Perish Song when all living foes already have a count.; Attack, Disable, burn, or sleep visible Ghost-type escape pieces immediately. Treat Soundproof, pivot moves, Shed Shell, and Eject Button as escape information only after the ability, move, or item is revealed in battle; do not inspect hidden player data for the state machine.; Prefer Protect, Substitute, King's Shield, Synthesis, Rage Powder, or a safe switch while a useful own-side count remains above one.; At count one, switch a perishing AI Pokemon when a legal reserve exists; current battle_ai_switch_items.c already provides this baseline and must be regression-tested in doubles.; Add a Phoebe-specific healthy-reserve selector that preserves Mega Gengar for a second cycle when defensible, chooses Amoonguss or Aegislash during the live clock, and chooses Mimikyu or Marshadow when the trap is broken or cleanup begins.; After the first opposing pair falls, allow Mega Gengar or Flutter Mane to re-enter and establish a second clock instead of drifting into generic damage AI.; Use Spectral Thief preferentially against a boosted target and Shadow Sneak only when priority changes the knockout.

### Fresh-context verdict

- Reviewer: `/root/phoebe_fresh_critic_final`
- Verdict: **ship**
- Is it sick and awesome? Yes. The apparent complexity resolves into one urgent question—break or escape Mega Gengar's clock—then evolves into guarded survival and direct spectral offense rather than a pile of strong Ghost types.
- Strongest moment: After the first countdown resolves, preserved Mega Gengar returns beside a guardian and starts a second circle, making the player's earlier targeting and escape-resource choices matter again.
- Weakest part: The opening is binary around Mega Gengar: a successful immediate double target can deflate the battle, while failing to answer it can erase a pair.
- Difficulty honesty: Mostly honest and strategy-driven. The modest offsets are not doing the heavy lifting; target priority, clock arithmetic, escape preservation, Protect prediction, and Marshadow's setup punishment are. Runtime tests must ensure the Gengar opening has a readable middle rather than only success or catastrophe.
...
- Primary question: Can the player use the guaranteed first-turn warning to manage Mega Glalie's later detonation, break Toxapex's trap-and-item transition, and still preserve an answer for final Chilling Neigh Glastrier?
- Primary mode: Mega Glalie and Toxapex open together, but Explosion is forbidden on global turn zero. Glalie first reveals Mega pressure through Double-Edge, Ice Shard, or Protect while Toxapex establishes one legal trap, poison, or Knock Off transition; only later can Baneful Bunker telegraph a safe detonation.
...
- Strict cap: 80
- Target / observed difficulty: **10 / None**
- Memory hook: Garchomp marks the field, mixed Dragapult and Scarf Hydreigon demand different answers, utility Zygarde interrupts greed, and special Mega Dragonite is the one reserved final climax.

### Exact proposed team

| # | Pokémon | Offset | Item | Ability | Moves | Role |
| ---: | --- | ---: | --- | --- | --- | --- |
| 1 | `SPECIES_GARCHOMP` | +1 | `ITEM_ROCKY_HELMET` | `ABILITY_ROUGH_SKIN` | `MOVE_STEALTH_ROCK`, `MOVE_EARTHQUAKE`, `MOVE_ROAR`, `MOVE_FIRE_FANG` | hazard lead, contact tax, reliable phazer, and accurate Steel coverage |
| 2 | `SPECIES_DRAGAPULT` | +2 | `ITEM_EXPERT_BELT` | `ABILITY_CLEAR_BODY` | `MOVE_DRAGON_DARTS`, `MOVE_SHADOW_BALL`, `MOVE_FLAMETHROWER`, `MOVE_U_TURN` | flexible mixed breaker and scouting pivot |
| 3 | `SPECIES_HYDREIGON` | +2 | `ITEM_CHOICE_SCARF` | `ABILITY_LEVITATE` | `MOVE_DRACO_METEOR`, `MOVE_DARK_PULSE`, `MOVE_FLASH_CANNON`, `MOVE_U_TURN` | special speed check, Fairy lure, and second pivot |
| 4 | `SPECIES_ZYGARDE_50_POWER_CONSTRUCT` | +2 | `ITEM_LEFTOVERS` | `ABILITY_POWER_CONSTRUCT` | `MOVE_EARTHQUAKE`, `MOVE_GLARE`, `MOVE_HAZE`, `MOVE_DRAGON_PULSE` | non-ace paralysis, boost-reset, and Ground-pressure utility |
| 5 | `SPECIES_DRAGALGE` | +2 | `ITEM_ASSAULT_VEST` | `ABILITY_ADAPTABILITY` | `MOVE_DRAGON_PULSE`, `MOVE_SLUDGE_BOMB`, `MOVE_SCALD`, `MOVE_THUNDERBOLT` | accurate special sponge and anti-Fairy wallbreaker |
| 6 | `SPECIES_DRAGONITE` | +3 | `ITEM_DRAGONINITE` | `ABILITY_MULTISCALE` | `MOVE_DRAGON_PULSE`, `MOVE_AIR_SLASH`, `MOVE_FLAMETHROWER`, `MOVE_ROOST` | sole Champions Mega showcase, special-category reversal, and final climax |

### Why this battle exists

- Primary question: Can the player exploit one visible Choice lock and shared Dragon weaknesses while adapting to mixed Dragapult and utility Zygarde, then arrive at the sole special Mega Dragonite climax with Multiscale already broken?
...
- Secondary mode: Power Construct Zygarde uses Glare and Haze as non-ace board utility, Assault Vest Dragalge punishes Fairy offense with accurate coverage, and special Mega Dragonite is held as the sole climax rather than sharing ace status.
- Difficulty rationale: Six optimized level-81 to level-83 Dragons create hazards, mixed damage, one exploitable Choice lock, paralysis and Haze utility, anti-Fairy pressure, and a single special Mega climax. Reduced offsets and accurate coverage make the target 10 depend on preservation and board-state adaptation rather than procession, raw levels, or misses.
- First-loss lesson: Identify Hydreigon as the one Choice lock, use the board state to predict whether Drake needs mixed Dragapult, Haze or Glare Zygarde, or anti-Fairy Dragalge, and preserve chip plus special bulk for the one true climax: Mega Dragonite.
- Intentional weakness: Every member remains a Dragon, only Hydreigon is Choice-locked, Dragapult has no setup or defensive item, Zygarde has no recovery or ace damage engine, Garchomp and Dragalge now use reliable but lower-ceiling coverage, and special Mega Dragonite is the sole climax whose Multiscale can be broken by hazards, status, weather, or preliminary chip.

### AI and evidence

- State machine: Phase A fixes Garchomp as the visible lead and conditionally establishes hazards or phazing. Phase B selects mixed Dragapult for flexible damage, Scarf Hydreigon for speed, utility Zygarde for Glare or Haze, or Dragalge for Fairy and special pressure according to the revealed board rather than array procession. Phase C preserves special Mega Dragonite as the default sole climax, breaks physical-wall expectations with accurate coverage, and Roosts only when Multiscale can matter.
- Selected references: `elite:giovanni-cischke:worlds-2025`, `smogon:gen8ou:002`, `showdown:gen9randomdoublesbattle:002`, `smogon:gen7uu:001`, `showdown:gen8randombattle:026`, `smogon:gen8ou:003`, `showdown:gen8randombattle:030`
- Required source work: Prefer Stealth Rock on turn zero only when Garchomp survives and hazards have value; use Roar against visible setup or a switch worth taxing, never as an accuracy gamble.; Treat Hydreigon as the one Choice-locked pivot and use U-turn or switching to escape an immunity; Dragapult remains flexible and should choose physical or special coverage from the visible matchup.; Select Zygarde only when Glare changes speed, Haze removes meaningful boosts, or its Ground pressure answers the board; it is utility, not the climax.; Select Dragalge when Fairy pressure or special bulk matters, using accurate Scald and Thunderbolt rather than gambling on Hydro Pump or Focus Blast.; Preserve healthy Mega Dragonite as the default final climax, Mega Evolve as Drake's sole Mega, and use Roost only when it restores a survivable Multiscale state.
...
- Spends: Mega Dragonite as Drake's signature, only Mega, and sole climax; Zygarde Power Construct as non-ace Dragon utility; the League's deliberate singles format shift; the campaign's definitive all-Dragon board-state selection and preservation test
...
- Releases: The old five-legendary Drake roster and three simultaneous setup clocks are released.; Mega Salamence is not locked to Drake and may appear elsewhere if its role does not duplicate Mega Dragonite.; Thousand Arrows is not reserved until its player-facing learnset source is made legal and explicitly approved.; Zygarde is released as a legendary ace and retained only as utility; Mega Dragonite owns the climax alone.
...
- Reviewer: `/root/wallace_fresh_critic_v2`
- Verdict: **ship**
- Is it sick and awesome? Yes. The three acts form one readable Champion escalation: survive royal rain, have the speed-war answer reversed by Ogerpon plus Palkia, then face an aggressive Ferrothorn plus Mega Milotic execution phase.
- Strongest moment: The player finally gets ahead of Kyogre and Tornadus, only for Ogerpon to redirect pressure while Palkia spends a denyable turn reversing the speed order.
- Weakest part: Trainer-aware reserve choreography could feel scripted if it ignores how the board actually collapsed, and Ferrothorn plus Recover Milotic can become slow if their direct-pressure rules fail.
- Difficulty honesty: Mostly honest and strategy-led. Raw levels raise the floor, but HP-sensitive Water Spout, interactable Tailwind, denyable Trick Room, redirection, and preserving a physical Milotic answer do the real work. The fight becomes fake-hard if those conditional behaviors are opaque or unconditional.
...
- Preserves: dedicated Perish and Shadow Tag for Phoebe and selected Aqua command fights; Ice stall-offense and toxic trapping for Glacia; Dark information warfare for Sidney; Dragon singles lure sequencing for Drake; Primal Reversion, Tera, Z-Moves, and Dynamax remain absent
...
- Advisory `PREMIUM_ITEM_REPETITION`: `{"ITEM_LEFTOVERS": ["ELITE_FOUR_DRAKE", "ELITE_FOUR_GLACIA", "ELITE_FOUR_PHOEBE"]}`
- Advisory `SIGNATURE_MOVE_REPETITION`: `{"MOVE_CLOSE_COMBAT": ["ELITE_FOUR_GLACIA", "ELITE_FOUR_PHOEBE", "ELITE_FOUR_SIDNEY"], "MOVE_KNOCK_OFF": ["CHAMPION_WALLACE", "ELITE_FOUR_GLACIA", "ELITE_FOUR_SIDNEY"], "MOVE_SHADOW_BALL": ["ELITE_FOUR_DRAKE", "ELITE_FOUR_GLACIA", "ELITE_FOUR_PHOEBE"], "MOVE_U_TURN": ["CHAMPION_WALLACE", "ELITE_FOUR_DRAKE", "ELITE_FOUR_SIDNEY"]}`
...
- Advisory `MODE_CLUSTER`: `{"choice-pressure": ["ELITE_FOUR_DRAKE", "ELITE_FOUR_GLACIA", "ELITE_FOUR_SIDNEY"], "fast-control": ["CHAMPION_WALLACE", "ELITE_FOUR_GLACIA", "ELITE_FOUR_SIDNEY"]}`
