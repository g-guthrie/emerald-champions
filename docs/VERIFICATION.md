...
Dialogue changes update the actual script, then rebuild. Failed compilation retains
...
## Trainer source synchronization

Edit `data/emerald_champions/emerald_champions_battle_teams.txt`, then run:
...
This compares authoring, plans, native parties and configured abilities;
it does not establish physical access or native battle quality.
...
The economy source inspection tool is `scripts/economy_reference.py`. It includes
