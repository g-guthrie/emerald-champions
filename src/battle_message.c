...
static const u8 sText_TargetTooHeavy[] = _("{B_DEF_NAME_WITH_PREFIX} is too heavy\nto be lifted!");
...
static const u8 sText_SymbiosisItemPass[] = _("{B_SCR_ACTIVE_NAME_WITH_PREFIX}'s {B_SCR_ACTIVE_ABILITY}\npassed {B_LAST_ITEM} to {B_EFF_NAME_WITH_PREFIX}!");
...
const u16 gFirstTurnOfTwoStringIds[] =
{
    ...
    [B_MSG_TURN1_SKY_DROP]      = STRINGID_TOOKPJMNINTOTHESKY,
};
...
void BufferStringBattle(u16 stringID)
{
    ...
    case STRINGID_USEDMOVE: // pokemon used a move msg
        if (gBattleMsgDataPtr->currentMove >= MOVES_COUNT)
            ...
            StringCopy(gBattleTextBuff3, gMoveNamesLong[gBattleMsgDataPtr->currentMove]);
        ...
        if (*src == PLACEHOLDER_BEGIN)
        {
            ...
            case B_TXT_CURRENT_MOVE: // current move name
                if (gBattleMsgDataPtr->currentMove >= MOVES_COUNT)
                    ...
                    toCpy = gMoveNamesLong[gBattleMsgDataPtr->currentMove];
                break;
            case B_TXT_LAST_MOVE: // originally used move name
                if (gBattleMsgDataPtr->originallyUsedMove >= MOVES_COUNT)
                    toCpy = sATypeMove_Table[gBattleStruct->stringMoveType];
                else
                    toCpy = gMoveNamesLong[gBattleMsgDataPtr->originallyUsedMove];
                break;
    ...
    *dst = EOS;
    while (src[srcID] != B_BUFF_EOS)
    {
        ...
        case B_BUFF_MOVE: // move name
            StringAppend(dst, gMoveNamesLong[T1_READ_16(&src[srcID + 1])]);
            srcID += 3;
            break;
...
{
    {0, sText_AarghAlmostHadIt, sText_BoxIsFull, sText_123Poof},
};
