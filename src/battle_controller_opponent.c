...
// One controller owns both send-outs only when there is a single opposing
// trainer. Two-opponent battles give each flank its own controller.
static bool32 TwoMonsAtSendOut(u32 battlerId)
{
    return (TwoIntroMons(battlerId)
...
static void Intro_WaitForShinyAnimAndHealthbox(void)
{
    ...
    if (!TwoMonsAtSendOut(gActiveBattler))
    {
...
static void SetOpponentMonData(u8 monId)
{
    ...
    case REQUEST_TOUGH_RIBBON_BATTLE:
        ...
        break;
    case REQUEST_NATURE_BATTLE:
        SetMonData(&gEnemyParty[monId], MON_DATA_NATURE, &gBattleResources->bufferA[gActiveBattler][3]);
        break;
    }
}
...
static void OpponentHandleChoosePokemon(void)
{
    ...
    {
        chosenMonId = GetMostSuitableMonToSwitchInto(); // switch after party mon is KO'ed?
