...
struct PairEvaluation
{
    ...
    s8 cottonDefenseDelta[MAX_BATTLERS_COUNT][MAX_MON_MOVES];
    ...
    // Reuse anchors for disjoint partner/item boosts, not a combined item tree.
    ...
    u32 *hp, u32 *speed, u32 *usedItems)
{
    ...
    if (effect == HOLD_EFFECT_RESTORE_PCT_HP && item == ITEM_SITRUS_BERRY)
    {
        // Preserve the native Sitrus-only odd-HP Belly Drum exception.
        threshold = (gBattleMons[target].maxHP + 1) / 2;
    ...
    {
        // HasEnoughHpToEatBerry uses floor-half for Gluttony, not Sitrus's
        // ceil-half. Do not grant an odd-HP full-health Drum a free Figy.
        u32 fraction = B_CONFUSE_BERRIES_HEAL >= GEN_7 ? 4 : 2;
        if (ability == ABILITY_GLUTTONY)
            fraction = 2;
        threshold = gBattleMons[target].maxHP / fraction;
        amount = PairNonDynamaxHP(ev, target, gBattleMons[target].maxHP)
...
static s32 PairPlanScore(enum BattlerId actor, const struct PairAction *action)
{
    ...
    if (IsStatRaisingMove(move) && (plan & EC_BATTLE_PLAN_SETUP)
     && action->score > AI_SCORE_DEFAULT && AI_CanAnyStatChange(actor, actor, move))
        return 35;
    return EC_PerishPlanScore(actor, move);
}
...
static void CachePairConditionalBoost(struct PairEvaluation *ev, enum BattlerId actor, u32 index, enum Move move)
{
    ...
    if (!IsBattlerAlive(actor) || (!partnerBoost && !itemBoost)
     || IsMoveUnusable(index, gBattleMons[actor].moves[index], gAiLogicData->moveLimitations[actor])
    ...
    for (enum BattlerId target = 0; target < gBattlersCount; target++)
         ...
         && gAiLogicData->simulatedDmg[actor][target][index].maximum)
        {
            ...
            struct SimulatedDamage boosted = itemBoost ? AI_CalcItemBoostDamage(&calc, actor, target, TRUE)
                : AI_CalcPartnerBoostDamage(&calc, actor, target, TRUE);
            struct SimulatedDamage unboosted = itemBoost ? AI_CalcItemBoostDamage(&calc, actor, target, FALSE)
...
static void CachePairMoveEffects(struct PairEvaluation *ev, enum BattlerId actor, u32 index)
{
    enum Move move = gBattleMons[actor].moves[index];
    enum Move executedMove = GetMoveEffect(move) == EFFECT_NATURE_POWER ? GetNaturePowerMove() : move;
    if (move == MOVE_COTTON_GUARD && IsBattlerAlive(actor))
    {
        ...
        struct StatChange st = {.onlyChecking = TRUE, .certain = TRUE, .stat = STAT_DEF};
        st.stage = GetAdjustedStatStage(GetStatStage(STAT_DEF, GetMoveAdditionalEffectById(move, 0)), cv.abilities[actor], FALSE);
        if (CanStatChange(&cv, &st))
            ev->cottonDefenseDelta[actor][index] = st.stage;
    }
    ...
    bool32 sleep = move == MOVE_SPORE || move == MOVE_SING;
    ...
    bool32 speedDrop = move == MOVE_ELECTROWEB || move == MOVE_ICY_WIND || thread;
    ...
    for (enum BattlerId target = 0; target < gBattlersCount; target++)
    {
        ...
        if (sleep)
        {
            ...
            if (DoesSubstituteBlockMove(actor, target, move)
             || (move == MOVE_SPORE && !IsAffectedByPowderMove(target, cv.abilities[target], cv.holdEffects[target]))
    ...
    for (u32 turn = 0; turn < gBattlersCount; turn++)
    {
         ...
         || (effect == EFFECT_HIT_ENEMY_HEAL_ALLY && IsBattlerAlly(actor, action->target)))
        {
            for (enum BattlerId target = 0; target < gBattlersCount; target++)
            {
                ...
                else if (GetMoveEffectArg_MoveProperty(move) == MOVE_EFFECT_FLORAL_HEALING && gFieldTimers.terrain == B_TERRAIN_GRASSY)
        ...
        if (move == MOVE_SHELL_SMASH || move == MOVE_COTTON_GUARD)
        {
            if (move == MOVE_COTTON_GUARD)
            {
                u32 cottonChance = actionChance[actor] / 100;
                if ((gBattleMons[actor].status1 & STATUS1_PARALYSIS)
                 && !(B_MAGIC_GUARD == GEN_4 && gAiLogicData->abilities[actor] == ABILITY_MAGIC_GUARD))
                    cottonChance = min(cottonChance, ev->paralysisActionChance / 100);
                if (!ev->cottonDefenseDelta[actor][action->index]
                 || PairMayBeSnatched(actor, actions, hp, acted, stopped)
                 || !PairScreenEffectApplies(applyEffects, cottonChance, effectChance))
                    continue;
                PairChangeStat(&statStage[actor][1], &statModifier[actor][1],
                    ev->cottonDefenseDelta[actor][action->index], 100, TRUE);
            }
            else
            ...
            // Shell Smash's future offense/Speed remains the native opinion.
        ...
        effect = GetMoveEffect(move);
        if (IsBattleMoveStatus(move) && !((move == MOVE_TOXIC_THREAD || move == MOVE_SPORE || move == MOVE_SING
        ...
        for (enum BattlerId original = 0; original < gBattlersCount; original++)
        {
            ...
            if (move == MOVE_SPORE || move == MOVE_SING)
            {
            ...
            u32 weatherPower = 100, oldWeatherPower = 100;
            if (weather != ev->weather)
            {
                weatherPower = PairWeatherPower(actor, target, move, weather);
                oldWeatherPower = PairWeatherPower(actor, target, move, ev->weather);
            }
            amount = amount * weatherPower / oldWeatherPower;
            minimumDamage = minimumDamage * weatherPower / oldWeatherPower;
            u32 worstDamage = damage.maximum * boost[actor] / 100 * weatherPower / oldWeatherPower;
             ...
             && !(acted & (1u << target)) && !IsBattleMoveStatus(actions[target].executedMove))
            {
                ...
                if (gAiLogicData->abilities[target] == ABILITY_STEAM_ENGINE
                 ...
                 && gAiLogicData->abilities[actor] != ABILITY_TURBOBLAZE)
                {
                    u32 gain = PairRaiseStage(&speedStage[target], MAX_STAT_STAGE);
            ...
            if (applyEffects && hp[target]
             && (move == MOVE_TOXIC_THREAD
                 || (damage.maximum && amount && (move == MOVE_ELECTROWEB || move == MOVE_ICY_WIND))))
            {
    ...
    for (enum BattlerId actor = 0; actor < gBattlersCount; actor++)
    {
        ...
        if (action->index != PAIR_IDLE
         && (GetMoveEffect(action->move) == EFFECT_REFLECT
             || (action->move == MOVE_COTTON_GUARD && (gBattleMons[actor].status1 & STATUS1_PARALYSIS))
            ...
            for (enum BattlerId target = 0; target < gBattlersCount; target++)
                if (ev->attackDropDelta[actor][action->index][target]
                 && (action->move == MOVE_CHARM || gAiLogicData->simulatedDmg[actor][target][action->index].maximum))
                    hasEffect = TRUE;
        if (action->index == PAIR_IDLE || (action->move != MOVE_ELECTROWEB && action->move != MOVE_ICY_WIND
            && action->move != MOVE_TOXIC_THREAD && action->move != MOVE_SPORE && action->move != MOVE_SING))
            continue;
        for (enum BattlerId target = 0; target < gBattlersCount; target++)
            if (((action->move == MOVE_SPORE || action->move == MOVE_SING)
...
static s32 EvaluatePairBoard(enum BattlerId actor, u32 noActionMask, struct PairAction *chosen, struct PairEvaluation *ev, bool32 refresh)
{
...
s32 AI_EvaluateDoublesPosition(enum BattlerId battler, u32 noActionMask)
{
    ...
    s32 score = EvaluatePairBoard(battler, noActionMask, NULL, ev, TRUE);
    ...
    for (u32 left = 0; left < count[0]; left++)
    {
        for (u32 right = 0; right < count[1]; right++)
        {
            ...
            for (u32 mega = 0; mega < 4; mega++)
            {
                ...
                if (!valid)
                    continue;
                s32 score = EvaluatePairBoard(actor, noActionMask, chosen, ev, mega != 0 || noActionMask != 0);
