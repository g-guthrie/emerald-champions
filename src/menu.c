...
static void *AllocDecompressedTileData(const void *src, u32 *copySize)
{
    u32 decompressedSize = GetDecompressedDataSize(src);
    void *ptr;

    if (!*copySize)
        *copySize = decompressedSize;
    // Tilesets reserve a full VRAM partition even when their blank trailing
    // tiles are omitted from the compressed payload. Keep that tail blank and
    // allocated until the asynchronous copy finishes. Partial copies still
    // need enough storage to decompress the entire source.
    ptr = AllocZeroed(decompressedSize > *copySize ? decompressedSize : *copySize);
    if (ptr)
...
void *DecompressAndCopyTileDataToVram(u8 bgId, const void *src, u32 size, u16 offset, u8 mode)
{
    ...
    {
        void *ptr = AllocDecompressedTileData(src, &size);
        if (ptr)
        {
