...
The inherited full-corpus workflow was retired. `test/` contains the focused runtime corpus, current project regressions and harness checks. Required runtime filters must execute named results with coherent counters and exit status; there are no historical test-count floors. Focused host checks can be run with:
