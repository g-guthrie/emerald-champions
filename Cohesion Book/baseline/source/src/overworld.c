...
void MigrateEmeraldChampionsCoreState(void)
{
    ...
    if (version == 2)
    {
        RestoreLostLegendaryEncounters();
        VarSet(VAR_EMERALD_CHAMPIONS_SAVE_VERSION, EMERALD_CHAMPIONS_SAVE_VERSION_CURRENT);
    }
    if (version == 2 || version == EMERALD_CHAMPIONS_SAVE_VERSION_CURRENT)
    {
        SetEmeraldChampionsPhysicalSignFlags();
        return;
    }
    ...
    if (version == 1)
    {
        // v1 has the current layout; preserve captures while restoring old losses.
        RestoreLostLegendaryEncounters();
        SetEmeraldChampionsPhysicalSignFlags();
        VarSet(VAR_EMERALD_CHAMPIONS_SAVE_VERSION, EMERALD_CHAMPIONS_SAVE_VERSION_CURRENT);
        return;
