...
{
    // Persistent forms such as Resolute Keldeo complete the same discovery.
...
// Migrate only encounters that the retired one-attempt system marked lost.
// Caught bits, earned rewards and unrelated map state are never reset.
void RestoreLostLegendaryEncounters(void)
{
    u8 *lost = gSaveBlock2Ptr->pokedex.lostLegendaryEncounters;
    for (enum LegendarySignId id = 0; id < LEGENDARY_SIGN_COUNT; id++)
    {
        if (!(lost[id / 8] & (1u << (id % 8))))
            continue;
        u16 flag = GetLegendarySignObjectFlag(id);
        if (flag != 0 && !IsLegendarySignCaught(id))
            FlagClear(flag);
    }
    for (u32 i = 0; i < ARRAY_COUNT(sNativeLegendaryEncounters); i++)
    {
        ...
        if (!(lost[index / 8] & (1u << (index % 8)))
         || GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT))
            continue;
        if (sNativeLegendaryDefeatedFlags[i])
            FlagClear(sNativeLegendaryDefeatedFlags[i]);
        if (sNativeLegendaryHideFlags[i])
            FlagClear(sNativeLegendaryHideFlags[i]);
        if ((species == SPECIES_LATIAS && VarGet(VAR_ROAMER_POKEMON) != 0)
         || (species == SPECIES_LATIOS && VarGet(VAR_ROAMER_POKEMON) == 0))
        {
            FlagClear(FLAG_DEFEATED_LATIAS_OR_LATIOS);
            FlagClear(FLAG_HIDE_SOUTHERN_ISLAND_UNCHOSEN_EON_DUO_MON);
        }
        for (u32 roamer = 0; roamer < ROAMER_COUNT; roamer++)
        {
            if (gSaveBlock1Ptr->roamer[roamer].species == species)
            {
                // The saved personality/IVs and route remain intact. A zero-HP
                // roamer is healed by its next encounter construction.
                gSaveBlock1Ptr->roamer[roamer].active = TRUE;
                gSaveBlock1Ptr->roamer[roamer].hp = 0;
                gSaveBlock1Ptr->roamer[roamer].statusA = 0;
                gSaveBlock1Ptr->roamer[roamer].statusB = 0;
            }
        }
    }
    memset(lost, 0, sizeof(gSaveBlock2Ptr->pokedex.lostLegendaryEncounters));
}
...
void BuildLegendarySignResearchMenu(void)
{
    ...
    {
        enum Species species = sNativeLegendaryEncounters[i];
        bool32 caught = GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), FLAG_GET_CAUGHT);
