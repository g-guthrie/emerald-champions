...
void ApplyTrainerLevelDifficulty(struct Pokemon *party)
{
    ...
    // Unbeaten route trainers keep pace with badge progress, including on
    // return visits. Preserve the authored gym levels and opening singles.
    // Apply the same difficulty offset after the floor: at cap 20 this is
    // Easy 16, Normal 18, Hard 20. Stronger authored opponents stay stronger.
    if ((gBattleTypeFlags & (BATTLE_TYPE_TRAINER | BATTLE_TYPE_DOUBLE)) == (BATTLE_TYPE_TRAINER | BATTLE_TYPE_DOUBLE)
     && !(gBattleTypeFlags & (BATTLE_TYPE_FRONTIER | BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED | BATTLE_TYPE_TRAINER_HILL))
     && GetCurrentMapBattleScene() != MAP_BATTLE_SCENE_GYM)
