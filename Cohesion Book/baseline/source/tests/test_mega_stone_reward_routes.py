...
class MegaStoneRewardRoutes(unittest.TestCase):
    ...
    def test_unattached_diancite_gift_does_not_count(self):
        def remove_pickup(payload):
            payload["object_events"] = [obj for obj in payload["object_events"]
                                        if obj["trainer_sight_or_berry_tree_id"] != "ITEM_DIANCITE"]

        with self.modified_map("CaveOfOrigin_DianciesRoom", remove_pickup):
            # The old Wallace gift still exists in scripts.inc, but no NPC
            # or trigger leads to it. Merely scanning giveitem is insufficient.
            self.assertNotIn("ITEM_DIANCITE", world_reward_sources())
