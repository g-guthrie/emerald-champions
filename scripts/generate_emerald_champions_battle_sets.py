...
def validate(entries: list[dict]) -> None:
    ...
    for entry in entries:
        ...
        assert entry["moves"][0] != "MOVE_NONE", (entry["species"], entry["name"])
...
def render_c(
    ...
    entries = []
    ranges = []
    for format_name, firsts, rest in (
        ("EC_BATTLE_FORMAT_DOUBLES", defaults, alternatives),
        ("EC_BATTLE_FORMAT_SINGLES", singles_defaults, singles_alternatives),
    ):
        by_species = {entry["species"]: [entry] for entry in firsts}
        for entry in rest:
            by_species[entry["species"]].append(entry)
        for species, choices in by_species.items():
            assert len(choices) <= 255 and len(entries) <= 65535, species
            ranges.append((format_name, species, len(entries), len(choices)))
            entries.extend(choices)
    assert len(entries) <= 65536
    names = {entry["name"]: i for i, entry in enumerate(entries)}
    lines = ["// Generated by scripts/generate_emerald_champions_battle_sets.py. Do not edit by hand."]
    for name, i in names.items():
        lines.append(f'static const u8 sEmeraldChampionsSetName_{i}[] = _("{name}");')
    lines.extend(["", "const struct EmeraldChampionsBattleSetChoice gEmeraldChampionsBattleSets[] =", "{"])
    for entry in entries:
        lines.append(f'    {{.name = sEmeraldChampionsSetName_{names[entry["name"]]}, .preset = {{')
        lines.extend(c_preset(entry))
        lines.append("    }},")
    lines.extend(["};", "", "const struct EmeraldChampionsBattleSetRange gEmeraldChampionsBattleSetRanges[EC_BATTLE_FORMAT_COUNT][NUM_SPECIES] =", "{"])
    for format_name, species, offset, count in ranges:
        lines.append(f"    [{format_name}][{species}] = {{.offset = {offset}, .count = {count}}},")
    lines.extend(["};", "", "static const bool8 sEmeraldChampionsPresetItems[ITEMS_COUNT] =", "{"])
    for item in sorted({entry["item"] for entry in entries} - {"ITEM_NONE"}):
