...
def main():
    ...
    findings=[
        ...
        f'ROSTER CLEANUP: {len(branches)} authored combat parties agree with nonempty native parties and Hoenn battle IDs. The {len(parties)-len(branches)-1} retired metadata records and TRAINER_NONE contain no combat loadouts. Native rematches are disabled. First-access ordering is tracked in data/emerald_champions/trainer-review-index.json; this export does not reuse a pre-restoration census.',
