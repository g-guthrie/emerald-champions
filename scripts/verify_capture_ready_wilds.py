...
check(
    "ordinary, Safari, scripted, roaming, and Frontier encounter provenance is explicit",
    battle_setup.count("gIsScriptedWildEncounter = FALSE;") >= 3
    and battle_setup.count("gIsScriptedWildEncounter = TRUE;") >= 8,
)
check(
    "the preset is applied after capture and before either party or PC storage",
    capture.index("ApplyVerdantBattleSetPreset") < capture.index("GiveMonToPlayer"),
)
check(
    "scripted, roaming, Frontier, and legendary or mythical captures are excluded",
    all(token in capture for token in (
        "!gIsScriptedWildEncounter",
        "BATTLE_TYPE_ROAMER",
        "BATTLE_TYPE_LEGENDARY",
        "BATTLE_TYPE_PIKE",
        "BATTLE_TYPE_PYRAMID",
        "!IsVerdantLegendarySpecies",
    )),
)
check(
    "capture presets change only moves, PP, nature, ability, and derived stats",
    ...
    and all(token not in function_body(runtime, "ApplyVerdantBattleSetPreset") for token in (
        "MON_DATA_HELD_ITEM",
