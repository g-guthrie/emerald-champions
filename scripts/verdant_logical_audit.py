...
questions = [
    ...
    ("Have redundant ordinary held-item rewards been replaced with finite progression?", '"redundantFixedRewardCount": 0' in read("docs/verdant_item_economy_ledger.json") and "fixed rewards contain zero free-vendor competitive items" in read("scripts/verdant_item_economy_audit.py") and "Berries" in read("docs/emerald_champions_progression_contract.md") and "evolution catalysts" in read("docs/emerald_champions_progression_contract.md")),
    ...
    ("Is the campaign genuinely mostly doubles while low-stakes routes get relief?", len(double_bodies) >= len(manifest["formats"]) * 0.64 and len(manifest["formats"]) - len(double_bodies) >= len(manifest["formats"]) * 0.25),
